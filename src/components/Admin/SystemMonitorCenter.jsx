...
export default function SystemMonitorCenter({
  ...
  const runHealthCheck = () => {
    ...
    setTimeout(() => {
      ...
      showToast('⚡ چک سلامت کلیه سرویس‌ها و دیتابیس با موفقیت انجام شد (سالم - 99.9٪)');
  ...
  const runAiSystemScan = () => {
    ...
    setTimeout(() => {
      ...
      showToast('🤖 آنالیز هوش مصنوعی سیستم و سرورها انجام شد');
    }, 1200);
  };
    ...
    <div className="space-y-4 text-xs">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-gradient-to-r from-blue-950/80 via-slate-900 to-slate-950 p-4 rounded-3xl border border-blue-500/40 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-white flex items-center gap-2">
              <span>مرکز مانیتورینگ سلامت سیستم و سرورها (System Monitor)</span>
              <span className="text-[10px] bg-emerald-500/20 text-emerald-300 font-mono px-2 py-0.5 rounded-full border border-emerald-500/30">
                ALL SYSTEMS OPERATIONAL
              </span>
            </h2>
            <p className="text-[11px] text-slate-400">
              بررسی سلامت دیتابیس Supabase، سرور LiveKit، سرویس Realtime، مصرف CPU و RAM، پایش هوش مصنوعی
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>{isHealthChecking ? 'در حال تست سرویس‌ها...' : `تست سلامت لحظه‌ای (آخرین: ${lastCheckTime})`}</span>
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2">
        {[
          { id: 'dashboard', label: '📊 داشبورد منابع سرور' },
          { id: 'services', label: '⚡ سلامت سرویس‌ها (Health Check)' },
          { id: 'logs', label: '📜 لاگ‌های لایو سیستم' },
          { id: 'ai_monitor', label: '🤖 هوش مصنوعی مانیتورینگ' }
        ].map(t => (
          <button
            key={t.id}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="p-4 rounded-3xl bg-slate-900 border border-slate-800 space-y-1 shadow-lg">
              <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center justify-between">
                <span>مصرف پردازنده (CPU Usage)</span>
            ...
            <div className="p-4 rounded-3xl bg-slate-900 border border-slate-800 space-y-1 shadow-lg">
              <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center justify-between">
                <span>فضای دیتابیس Supabase</span>
            ...
            <div className="p-4 rounded-3xl bg-slate-900 border border-slate-800 space-y-1 shadow-lg">
              <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center justify-between">
                <span>پناه‌پهنای باند (Bandwidth)</span>
              ...
              <table className="w-full text-right border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-950 text-slate-400 border-b border-slate-800 text-[11px]">
                    <th className="p-3.5">نام سرویس</th>
                    <th className="p-3.5">دسته‌بندی</th>
                    <th className="p-3.5">زمان پاسخ (Latency)</th>
                    <th className="p-3.5">آپتایم (Uptime)</th>
          ...
          <div className="p-5 rounded-3xl bg-gradient-to-br from-slate-900 via-slate-950 to-slate-950 border border-cyan-500/40 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-white text-base">دستیار پایش هوشمند زیرساخت و سرورها (AI Infrastructure Monitor)</h3>
                  <p className="text-xs text-slate-400">شناسایی کندی کوئری‌ها، قطعی احتمالی، نشت حافظه و تخمین هزینه‌های سرور</p>
                </div>
              </div>
              ...
              >
                ...
                <span>{isAiAnalyzingSystem ? 'در حال پایش عمیق سرورها...' : 'اجرای اسکن AI زیرساخت'}</span>
              </button>
            </div>
            ...
            <div className="p-3 rounded-2xl bg-blue-500/10 border border-blue-500/30 text-blue-300 text-[11px] flex items-center gap-2">
              ...
              <span>
                <strong>محدودیت امنیتی AI:</strong> هوش مصنوعی مانیتورینگ فقط وظیفه تحلیل و گزارش‌دهی را دارد و هیچ‌گونه دسترسی به ریست سرور یا حذف داده‌ها ندارد.
              </span>
            </div>
              ...
              <div className="space-y-3 pt-2 animate-fadeIn">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-3.5 rounded-2xl bg-slate-900 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-bold">امتیاز سلامت کلی سیستم</span>
                    <span className="text-lg font-black text-emerald-400 font-mono">{aiSystemReport.overallHealthScore}</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-slate-900 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-bold">امتیاز کارایی سرور</span>
                    <span className="text-lg font-black text-cyan-300 font-mono">{aiSystemReport.performanceScore}</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-slate-900 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-bold">پیش‌بینی هزینه ماهانه سرور</span>
                    <span className="text-lg font-black text-amber-300 font-mono">{aiSystemReport.estimatedServerCosts}</span>
                  </div>
                </div>

                <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
                  <h4 className="font-bold text-white text-xs">توصیه‌های بهینه‌سازی AI:</h4>
