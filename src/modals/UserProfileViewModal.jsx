...
export default function UserProfileViewModal({
  ...
  const toggleFollow = () => {
    ...
    showToast(nextState ? `با موفقیت ${userName} را دنبال کردید 👤` : `کاربر ${userName} آنفالو شد`);
  };
  ...
  const toggleLike = () => {
    ...
    showToast(nextState ? `💖 لایک برای ${userName} ارسال شد` : `لایک پس گرفته شد`);
  };

  const triggerSuperLike = () => {
    setIsSuperLiked(true);
    showToast(`🌟 سوپرلایک طلایی برای ${userName} فرستاده شد!`);
  };
  ...
  const submitReport = () => {
    ...
    setReportNotes('');
    showToast('🚩 گزارش تخلف با موفقیت ثبت شد و بررسی خواهد شد');
  };
  ...
  const handleAdminBanToggle = () => {
    ...
    showToast(nextState ? `🚫 کاربر ${userName} مسدود شد` : `کاربر ${userName} از مسدودی خارج شد`);
  };
  ...
  const handleAdminVerifyToggle = () => {
    ...
    showToast(nextState ? `✅ نشان تایید برای ${userName} فعال شد` : `نشان تایید غیرفعال شد`);
  };
  ...
  const handleAdminStreamerToggle = () => {
    ...
    showToast(nextState ? `🎥 مقام استریمر به ${userName} اعطا شد` : `مقام استریمر لغو شد`);
  };
        ...
        <div className="px-5 pb-6 overflow-y-auto flex-1 space-y-5 -mt-14 relative z-10 scrollbar-none">
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="flex items-center gap-2 pb-1">
              ...
              >
                ...
                <span>{isFollowing ? loc('دنبال می‌کنید', 'Following') : loc('دنبال کردن', 'Follow')}</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 pt-2">
            ...
            >
              ...
              <span>{loc('تماس صوتی', 'Voice Call')}</span>
            </button>
            ...
            >
              ...
              <span>{loc('ارسال هدیه', 'Send Gift')}</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 p-3 bg-slate-950/80 rounded-2xl border border-slate-800 text-center text-xs">
            <div>
              ...
              <span className="text-[10px] text-slate-400">{loc('دنبال‌کننده', 'Followers')}</span>
            </div>
            <div>
              <span className="block font-black text-pink-400 text-sm">{likesCount.toLocaleString()}</span>
              <span className="text-[10px] text-slate-400">{loc('لایک‌ها', 'Likes')}</span>
            </div>
            <div>
              <span className="block font-black text-cyan-400 text-sm">{publicPhotos.length}</span>
              <span className="text-[10px] text-slate-400">{loc('عکس/فیلم', 'Media')}</span>
            </div>
            <div>
              <span className="block font-black text-amber-400 text-sm">Lv.22</span>
              <span className="text-[10px] text-slate-400">{loc('سطح', 'Level')}</span>
            </div>
          </div>
            ...
            <div className="p-4 rounded-3xl bg-gradient-to-r from-pink-950/80 via-slate-900 to-slate-900 border border-pink-500/40 space-y-2">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-pink-300 font-black text-xs">
                  ...
                  <span>{loc('میزبان فعال استریم و تماس ویدئویی', 'Active Live Streamer')}</span>
                </span>
            ...
            <div className="p-4 rounded-3xl bg-rose-950/60 border border-rose-500/50 space-y-3">
              <div className="flex items-center justify-between border-b border-rose-500/30 pb-2">
                <span className="text-xs font-black text-rose-300 flex items-center gap-1.5">
                  ...
                  <span>ابزارهای نظارت مدیر (Super Admin Tools)</span>
                </span>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                >
                  ...
                  <span>{isBanned ? 'رفع مسدودی' : 'مسدود کردن'}</span>
                </button>
                ...
                >
                  ...
                  <span>{isVerified ? 'لغو نشان' : 'اعطای تایید'}</span>
                </button>
                ...
                >
                  ...
                  <span>{isStreamer ? 'لغو استریمر' : 'اعطای لایو'}</span>
                </button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3 pt-1">
            <div className="flex items-center gap-2 p-1 bg-slate-900 rounded-2xl border border-slate-800">
              {[
                { id: 'overview', label: loc('آلبوم تصاویر', 'Photos') },
                { id: 'about', label: loc('درباره و علاقه‌ها', 'About') }
              ].map(t => (
                <button
                  key={t.id}
            ...
            {activeTab === 'about' && (
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3 text-xs animate-fadeIn">
                <div className="space-y-1">
                  <span className="text-slate-400 font-bold">زبان‌ها:</span>
                  <p className="text-white font-medium">Persian, English, Turkish</p>
                </div>
                <div className="space-y-1">
                  <span className="text-slate-400 font-bold">علاقه‌مندی‌ها:</span>
        ...
        <div className="fixed inset-0 z-[80] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-slate-900 rounded-3xl border border-slate-800 p-5 space-y-4 shadow-2xl dir-rtl">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300">علت گزارش:</label>
              <select
                value={reportReason}
                onChange={(e) => setReportReason(e.target.value)}
                className="w-full p-3 rounded-2xl bg-slate-950 border border-slate-800 text-white text-xs outline-none"
              >
                <option value="fake_account">اکانت جعلی یا فیک</option>
                <option value="spam">اسپم و تبلیغات مزاحم</option>
                <option value="harassment">مزاحمت و رفتار نامناسب</option>
                <option value="fake_photos">تصاویر غیرواقعی</option>
                <option value="other">سایر موارد</option>
              </select>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300">توضیحات تکمیلی (اختیاری):</label>
              <textarea
                value={reportNotes}
                onChange={(e) => setReportNotes(e.target.value)}
                placeholder="توضیح کوتاه درباره علت گزارش..."
